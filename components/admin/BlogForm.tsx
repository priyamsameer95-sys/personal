...
export default function BlogForm({ editItem, onSuccess, onDirtyChange }: BlogFormProps) {
  ...
  useEffect(() => {
    onDirtyChange(true);
  }, [title, summary, bodyMarkdown, downloadable, images, onDirtyChange]);
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (images.length + files.length > 3) {
      setError('Maximum 3 images allowed for Blog posts.');
      return;
    }
  ...
  const handleSubmit = async (status: 'draft' | 'published') => {
    try {
      ...
      if (images.length > 3) throw new Error('Maximum 3 images allowed for Blog');
      ...
      for (const img of images) {
        if (img.file) {
          const blob = await upload(img.file.name, img.file, {
            ...
            clientPayload: JSON.stringify({ type: 'blog' }),
          });
          ...
          uploadedCount++;
          if (totalToUpload > 0) setUploadProgress((uploadedCount / totalToUpload) * 70);
        } else {
          mediaPayload.push({
            id: img.id,
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-[#1A1A1A]">Blog Post</h2>
        ...
        <div>
          <label className="block text-sm font-semibold text-[#525252] mb-1">Summary (excerpt)</label>
          <textarea 
            value={summary} 
            onChange={e => setSummary(e.target.value)}
            className="w-full p-3 border border-[#E5E5E5] rounded-xl bg-[#FAFAFA] text-[#1A1A1A] min-h-[80px] focus:outline-none focus:ring-2 focus:ring-[#1A1A1A]"
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {images.length < 3 && (
              <div className="border-2 border-dashed border-[#E5E5E5] rounded-xl p-4 text-center hover:bg-[#FAFAFA] transition cursor-pointer">
                <input 
                  type="file" 
                ...
                />
                <label htmlFor="blog-image-upload" className="cursor-pointer flex items-center justify-center gap-2">
                  <span className="material-symbols-rounded text-xl text-[#8C8C8C]">add_photo_alternate</span>
                  <span className="text-[#1A1A1A] font-semibold text-sm">Add Image (Optional)</span>
                </label>
              </div>
            )}
          </div>
        ...
        <div>
          ...
          <WordCounter text={bodyMarkdown} limit={2500} />
        </div>
      ...
      {isUploading && <ProgressBar progress={uploadProgress} message="Saving Post..." />}
